...
const estadoCompraAEliminar = computed(() => props.estadoCompra);

// Función para cancelar y emitir el evento al padre
const handleEliminar = (estadoCompraAEliminar: any) => {
    emit("eliminar", estadoCompraAEliminar);
}
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-black/50">
      <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
        ...
        <p class="mb-6 text-gray-600">
          ¿Estás seguro de que deseas eliminar este estado de compra con razon social: {{estadoCompraAEliminar.razon_social}}
          y con identificador: {{estadoCompraAEliminar.id}}?
        </p>
        <div class="flex justify-end gap-2">
          ...
          <button
            ...
            @click="handleEliminar(estadoCompraAEliminar)"
          >
            Eliminar
          </button>
        </div>
